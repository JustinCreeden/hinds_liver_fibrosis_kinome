...
kea3_files = dir(here::here("kea3_datasets"), full.names = TRUE)
...
kegg_graphs = readRDS(here::here("rmflight_reports", "kegg_pathway_graphs.rds"))
all_entrez = keys(org.Hs.eg.db)
org_hs_df = select(org.Hs.eg.db, keys = all_entrez, columns = "SYMBOL")

specific_to_collapsed = read.table(here::here("rmflight_reports", "full_nodes_collapsed.csv"), sep = "\t", header = TRUE)
...
set.seed(1234)
out_graph = ggraph(multi_collapsed_uka, "graphopt") +
geom_edge_link(arrow = arrow(length = unit(2, 'mm')),
                end_cap = circle(10, "mm")) +
...
```{r export_pathway}
plot_list = list(collapsed_uka = out_graph)
export_plots(plot_list, "rmflight_reports/kegg_pathway_plot_2021-11-09.pptx")
```
